...
export function validateChanges(scape: Scape, input: unknown[], allowedTypes: string[] = []) {
  ...
  for (const raw of input) {
    ...
    if (a.type === "CreateObject") {
      if (
        Object.hasOwn(next.objects, a.id) ||
        ["__proto__", "constructor", "prototype"].includes(a.id)
      )
        throw new Error(`duplicate_id: ${a.id}`);
      if (allowedTypes.length && !allowedTypes.includes(a.objectType))
        throw new Error(`unsupported_type: ${a.objectType}`);
      ...
      if (
        !Object.hasOwn(next.objects, a.from) ||
        !Object.hasOwn(next.objects, a.to) ||
        a.from === a.to
      )
        throw new Error("invalid_relationship");
    } else if (a.type === "DisconnectObjects" && !Object.hasOwn(next.relationships, a.id))
      throw new Error(`not_found: ${a.id}`);
