...
/**
 * The read-only half of the registry — `ViewObject`, `ViewPlugin`, and the `view.ts` glob —
 * lives in `./viewRegistry`. It is a separate module because both globs are eager, so a shared
 * file would have made every viewer import pull the editor's plugins, and with them the store,
 * into the public viewer's bundle. See the header there.
 */
